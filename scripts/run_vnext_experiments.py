...
def main() -> None:
    ...
    # Load base config
    base_cfg = load_config(suite.base_config_path)
    ...
    for exp_idx, experiment in enumerate(suite.experiments, 1):
        ...
        effective_cfg = deep_merge_dicts(base_cfg, experiment.overrides)
        ...
        if args.dry_run:
            ...
            entry: Dict[str, Any] = {
                ...
                "grf_axes": "fz" if effective_cfg.get("model", {}).get("type", "fz") == "fz" else "3d",
